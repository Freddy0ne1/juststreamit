(() => {
  ...
  // ---------------------------------------------
  // AFFICHAGE D’UNE CARTE ACTION
  // ---------------------------------------------

  // Met l’affiche (image) du film dans l’<img> de la carte aventure.
  const setActionImage = async (id, selector = ".actionMovies .image-1") => {
  ...
  const attachIdToActionButton = (fromTitleEl, movieId) => {
  ...
  const fillActionCard = async (movie, index) => {
    const titleSel = `.actionMovies .titleMovie-${index}`;
    const imageSel = `.actionMovies .image-${index}`;
    ...
    // 2) Image
    await setActionImage(movie.id, imageSel);

    // 3) Bouton Détails
    if (titleEl) attachIdToActionButton(titleEl, movie.id);
  };
  ...
  const loadActionMovies = async () => {
    const r = await fetch(urlActionMovies); // on appelle l’API liste
    ...
    for (let i = 1; i <= n; i++) {
      ...
      await fillActionCard(movie, i); // on remplit la carte
    }
  };
